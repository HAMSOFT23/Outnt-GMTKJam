...
namespace MaxyGames.UNode.Editors {
	public static class SyntaxAnalizer {
		struct ReplaceLocation {
			public int line;
			public int column;
		}
		...
		public static string AnalizeCode(string source) {
			if(uNodePreference.preferenceData.generatorData.analyzeScript) {
				if(uNodePreference.preferenceData.generatorData.removeUnnecessaryCode) {
					source = RemoveUnusedParenthesized(source);
				}
				if(uNodePreference.preferenceData.generatorData.formatScript) {
		...
		public static string RemoveUnusedParenthesized(string source) {
			var root = RoslynUtility.GetSyntaxTree(source, out var model);

			List<ParenthesizedExpressionSyntax> syntaxToReplaces = new();
			root.DescendantNodes(node => {
				if(node is ParenthesizedExpressionSyntax syntax && syntax.CanRemoveParentheses(model)) {
					syntaxToReplaces.Add(syntax);
				}
				return true;
			}).LastOrDefault();

			syntaxToReplaces.Sort((a, b) => {
			...
			List<int> codeToRemoves = new();
			foreach(var syntax in syntaxToReplaces) {
				codeToRemoves.Add(syntax.OpenParenToken.SpanStart);
				codeToRemoves.Add(syntax.CloseParenToken.SpanStart);
			}

			var strs = root.ToFullString();
			codeToRemoves.Sort((x, y) => {
				if(x > y) {
					return -1;
				}
				else if(x < y) {
			...
			var result = root.ToFullString().ToList();
			foreach(var idx in codeToRemoves.Distinct()) {
				result.RemoveAt(idx);
			}
	...
	internal static class ParenthesizedExpressionSyntaxExtensions {
		public static bool CanRemoveParentheses(this ParenthesizedExpressionSyntax node, SemanticModel semanticModel) {
			...
			if((node.IsParentKind(SyntaxKind.EqualsValueClause) && ((EqualsValueClauseSyntax)node.Parent).Value == node) ||
				...
				(node.IsParentKind(SyntaxKind.ForEachStatement) && ((CommonForEachStatementSyntax)node.Parent).Expression == node) ||
		...
		private static readonly ObjectPool<Stack<SyntaxNode>> s_nodeStackPool = new ObjectPool<Stack<SyntaxNode>>(() => new Stack<SyntaxNode>(), val => val.Clear(), val => val.Clear());
			...
			if(precedence > parentPrecedence) {
				...
				if(parentExpression is BinaryExpressionSyntax parentBinaryExpression) {
					...
					if(IsAssociative(parentBinaryExpression.Kind()) &&
						...
						return !RoslynExtensions.IsSafeToChangeAssociativity(
							node.Expression, parentBinaryExpression, semanticModel);
					}
